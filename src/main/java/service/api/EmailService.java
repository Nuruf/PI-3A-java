...
public class EmailService {
    ...
    public EmailService() {
        ...
        String smtpUsername = EnvConfig.get("SMTP_USERNAME", "");
        String smtpPassword = EnvConfig.get("SMTP_PASSWORD", "");
        ...
        this.enabled = smtpUsername != null
                && !smtpUsername.isEmpty()
                && !smtpUsername.equals("your_email@gmail.com")
                && smtpPassword != null
    ...
    private boolean sendEmail(String toEmail, String toName, String subject, String htmlBody) {
        if (!enabled) {
            System.out.println("Email (simulation): " + subject + " → " + toEmail);
            return true;
        }

        if (toEmail == null || toEmail.isEmpty() || !toEmail.contains("@")) {
            System.err.println("Invalid email: " + toEmail);
            return false;
        }

        try {
            String smtpUsername = EnvConfig.get("SMTP_USERNAME", "");
            String smtpFromName = EnvConfig.get("SMTP_FROM_NAME", "RH - Momentum");

            MimeMessage message = new MimeMessage(session);
            message.setFrom(new InternetAddress(smtpUsername, smtpFromName));
            ...
            System.err.println("Email auth failed - check .env: SMTP_USERNAME and SMTP_PASSWORD");
            return false;
    ...
    public CompletableFuture<Boolean> sendNewDemandeToRH(
        ...
        return CompletableFuture.supplyAsync(() -> {
            ...
            String html = buildNewDemandeHtml(employeeName, employeeEmail, demandeTitre, demandeType, priorite);
            return sendEmail(EnvConfig.get("RH_EMAIL", "rh@company.com"), "Service RH", subject, html);
        });
    }
    ...
    private String buildNewDemandeHtml(String employeeName, String employeeEmail,
        ...
        if ("HAUTE".equalsIgnoreCase(priorite)) prioriteColor = "#e74c3c";
        else if ("BASSE".equalsIgnoreCase(priorite)) prioriteColor = "#27ae60";
        ...
        return "<!DOCTYPE html><html><head><meta charset='UTF-8'></head><body style='font-family:Segoe UI,Arial,sans-serif;margin:0;padding:0;background:#f4f4f4'>"
                ...
                + "    " + escapeHtml(EnvConfig.get("COMPANY_NAME", "Momentum")) + " — Système RH"
                + "  </div>"
                + "</div></body></html>";
    }
    ...
    public CompletableFuture<Boolean> sendStatusChangeToEmployee(
        ...
        return CompletableFuture.supplyAsync(() -> {
            String subject = "Mise a jour: " + demandeTitre;
    ...
    public CompletableFuture<Boolean> sendDemandeCancelledToRH(
        ...
        return CompletableFuture.supplyAsync(() -> {
            String subject = "Demande Annulee: " + demandeTitre;
            String html = buildCancellationEmailForRH(employeeName, employeeEmail, demandeTitre, demandeType, demandePriorite);
            return sendEmail(EnvConfig.get("RH_EMAIL", "rh@company.com"), "Service RH", subject, html);
        });
    }
    ...
    private String buildCancellationEmailForRH(
        ...
        return "<!DOCTYPE html>"
                ...
                + "      <table style='width:100%;border-collapse:collapse'>"
                + "        <tr><td style='padding:10px 0;border-bottom:1px solid #eee;color:#888;width:130px'>📌 Titre</td>"
                + "            <td style='padding:10px 0;border-bottom:1px solid #eee;font-weight:bold;color:#2c3e50'>" + escapeHtml(demandeTitre) + "</td></tr>"
                + "        <tr><td style='padding:10px 0;border-bottom:1px solid #eee;color:#888'>📂 Type</td>"
                + "            <td style='padding:10px 0;border-bottom:1px solid #eee;color:#2c3e50'>" + escapeHtml(demandeType) + "</td></tr>"
                + "        <tr><td style='padding:10px 0;border-bottom:1px solid #eee;color:#888'>⚡ Priorité</td>"
                ...
                + "              </span></td></tr>"
                + "        <tr><td style='padding:10px 0;color:#888'>🚫 Statut</td>"
                + "            <td style='padding:10px 0'><span style='background:#fadbd8;color:#c0392b;padding:4px 12px;border-radius:12px;font-size:12px;font-weight:bold'>Annulée</span></td></tr>"
                + "      </table>"
                + "    </div>"
                + "    <div style='background:#f8f9fa;padding:20px;border-radius:8px;margin-bottom:20px'>"
                + "      <h3 style='margin:0 0 15px;color:#2c3e50;font-size:16px'>👤 Informations employé</h3>"
                + "      <table style='width:100%;border-collapse:collapse'>"
                + "        <tr><td style='padding:10px 0;border-bottom:1px solid #eee;color:#888;width:130px'>👤 Nom</td>"
                + "            <td style='padding:10px 0;border-bottom:1px solid #eee;color:#2c3e50;font-weight:bold'>" + escapeHtml(employeeName) + "</td></tr>"
                + "        <tr><td style='padding:10px 0;border-bottom:1px solid #eee;color:#888'>📧 Email</td>"
                + "            <td style='padding:10px 0;border-bottom:1px solid #eee;color:#2c3e50'>" + escapeHtml(employeeEmail) + "</td></tr>"
                + "        <tr><td style='padding:10px 0;color:#888'>🕐 Date annulation</td>"
                + "            <td style='padding:10px 0;color:#2c3e50'>" + currentDate + "</td></tr>"
                + "      </table>"
                + "    </div>"
                ...
                + "        💡 <em>Cette demande a été annulée par l'employé car elle n'avait pas encore été traitée. Aucune action n'est requise.</em>"
                + "      </p>"
                + "    </div>"
                + "  </div>"
                + "  <div style='background:#f8f9fa;padding:20px;text-align:center;border-top:1px solid #eee'>"
                + "    <p style='margin:0;color:#999;font-size:12px'>📧 Email automatique du système de gestion des demandes</p>"
                + "    <p style='margin:5px 0 0;color:#bbb;font-size:11px'>" + escapeHtml(EnvConfig.get("COMPANY_NAME", "Momentum")) + " — Système RH</p>"
                + "  </div>"
                + "</div>"
                + "</body>"
                + "</html>";
    ...
    public CompletableFuture<Boolean> sendDemandeCancelledConfirmation(
        ...
        return CompletableFuture.supplyAsync(() -> {
            ...
            String subject = "Confirmation: " + demandeTitre;
            String html = buildCancellationConfirmationEmail(employeeName, demandeTitre, demandeType);
            return sendEmail(employeeEmail, employeeName, subject, html);
        });
    }
    ...
    private String buildCancellationConfirmationEmail(
            String employeeName, String demandeTitre, String demandeType) {
        ...
        return "<!DOCTYPE html>"
                ...
                + "        <tr><td style='padding:10px 0;border-bottom:1px solid #eee;color:#888'>📂 Type</td>"
                + "            <td style='padding:10px 0;border-bottom:1px solid #eee;color:#2c3e50'>" + escapeHtml(demandeType) + "</td></tr>"
                + "        <tr><td style='padding:10px 0;border-bottom:1px solid #eee;color:#888'>🚫 Statut</td>"
                + "            <td style='padding:10px 0;border-bottom:1px solid #eee'><span style='background:#fadbd8;color:#c0392b;padding:4px 12px;border-radius:12px;font-size:12px;font-weight:bold'>Annulée</span></td></tr>"
                + "        <tr><td style='padding:10px 0;color:#888'>🕐 Date</td>"
                + "            <td style='padding:10px 0;color:#2c3e50'>" + currentDate + "</td></tr>"
                + "      </table>"
                + "    </div>"
                + "    <div style='background:#e8f4fd;padding:15px;border-radius:8px;border-left:4px solid #3498db;margin-bottom:20px'>"
                + "      <p style='margin:0;color:#2980b9;font-size:13px'>ℹ️ Le service RH a été notifié de cette annulation.</p>"
                + "    </div>"
    ...
    public CompletableFuture<Boolean> sendDemandeCreatedConfirmation(
        ...
        return CompletableFuture.supplyAsync(() -> {
            ...
            String html = buildDemandeCreatedConfirmationEmail(employeeName, demandeTitre, demandeType, demandePriorite);
            return sendEmail(employeeEmail, employeeName, subject, html);
        });
    }

    private String buildDemandeCreatedConfirmationEmail(
            String employeeName, String demandeTitre,
            String demandeType, String demandePriorite) {
        ...
        return "<!DOCTYPE html>"
                ...
                + "            </td></tr>"
                + "        <tr><td style='padding:10px 0;border-bottom:1px solid #eee;color:#888'>📊 Statut</td>"
                + "            <td style='padding:10px 0;border-bottom:1px solid #eee'><span style='background:#3498db;color:white;padding:4px 12px;border-radius:12px;font-size:12px;font-weight:bold'>Nouvelle</span></td></tr>"
                + "        <tr><td style='padding:10px 0;color:#888'>🕐 Date</td>"
                + "            <td style='padding:10px 0;color:#2c3e50'>" + currentDate + "</td></tr>"
                + "      </table>"
                + "    </div>"
    ...
    public void testConnection() {
        ...
        if (enabled) {
            sendEmail(
                    EnvConfig.get("RH_EMAIL", "rh@company.com"),
                    "Test",
                    "Test Email",
                    "<p>Email service is working</p>"
            );
        } else {
            System.out.println("Email service disabled. Set SMTP_USERNAME and SMTP_PASSWORD in .env");
        }
    }
    ...
    public CompletableFuture<Boolean> sendTestEmail(String toEmail) {
        return CompletableFuture.supplyAsync(() -> {
            ...
            String html = "<html><body style='font-family:Arial'><p>Test email sent at " +
                    new java.util.Date() + "</p><p style='color:#999;font-size:12px'>" +
                    escapeHtml(EnvConfig.get("COMPANY_NAME", "Momentum")) + "</p></body></html>";
            return sendEmail(toEmail, "Test", subject, html);
        });
    }
}
